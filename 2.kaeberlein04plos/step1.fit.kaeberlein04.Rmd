...
author: "h qin"
date: "June 28 2017 - `r format(Sys.time(), '%d %B, %Y')`"
output:
...
my.labels = c("BY4742",                  "SIR2_Overexpression",             'sir2',     'sir2', 
              "BY4742", "fob1",  "hxk2", "fob1 hxk2");
...
#over probility mass function with binomial model
see  http://hongqinlab.blogspot.com/2013/12/binomial-mortailty-model.html
 m = R ( 1 + t/t0)^(n-1)
 s = exp( (R t0/n)*(1 - (1+t/t0)^n ) )  
 pdf = s*m
...
for( i in 1:length(report[,1])){
  ...
  h= hist(my.data, br= seq(0, max(my.data), length.out = sqrt(max(my.data))+7 ));
  
  figname = paste( 'plots/', my.labels[i], '.jpg', sep='');
  jpeg(figname, width=480, height=480, quality=100);
  
  hist(my.data, br= seq(0, max(my.data), length.out = sqrt(max(my.data))+7 ),
       probability = TRUE, col='gray', border='white', xlab='RLS', ylab='probability',
       main=my.labels[i], ylim=c(0, max(h$density)*1.1), xlim=c(0, max(h$mids)*1.1))
  #plot( h$density ~ h$mids, main=my.strains[i], xlab="RLS",ylab="density")
  #par(new=TRUE);
