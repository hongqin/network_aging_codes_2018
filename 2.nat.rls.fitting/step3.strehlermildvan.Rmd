...
ggsave( "plots/strehler-mildvan.pdf", device="pdf", width=5, height=3)
ggsave( "plots/strehler-mildvan.jpg", device="jpeg", width=5, height=3)
```


```{r SM-plot}
#pdf("plots/strehler-mildvan_backup.pdf", width=5.2, height=5.2)
...
report <- report %>% as_tibble %>% mutate( 
  G = GompGFlex  #rename columns for easy plot
)

ggplot(report, aes(x=t0, y = GompRFlex))  + 
  geom_point(aes(size=G)) +
  labs( x = "t0", y="R") + 
  scale_y_continuous(trans='log10', limits = c(1E-4, 1E-2), breaks=c( 0.0001, 0.001, 0.005, 0.01),labels=c(0.0001, 0.001, 0.005, 0.01)) +
  geom_smooth(method = 'lm', se=FALSE, formula = y ~ x, linetype = 3, colour='red') + 
 annotate("text", x = 60, y = 2E-4, label = paste("~R^2==~",0.38,"~p==~", 0.014), parse=TRUE )

ggsave( "plots/log10R-t10.pdf", device="pdf", width=5, height=3)
ggsave( "plots/log10R-t10.jpg", device="jpeg", width=5, height=3)
```


```{r log10R-t0-plot}
#pdf("plots/log10R-t10_bakcup.pdf", width=5.2, height=5.2)
...
# Mediation test on Gflex <--n <-- RFlex (non significant)
```{r}
library(mediation)
report$log10GompRFlex = log10(report$GompRFlex)

med.fit = lm(n ~ log10GompRFlex, data=report)  #mediator ~ treatment
summary(med.fit)

out.fit = lm(GompGFlex ~ n + log10GompRFlex, data=report)  #outcome ~ mediator + treatment
summary(out.fit)

med.out <- mediate(med.fit, out.fit, treat = "log10GompRFlex", mediator = "n", robustSE = TRUE, sims = 100) #mediator test
summary(med.out)
...
# Mediation test on Rflex <--n <-- GFlex (non-signifcant: n~G)
```{r}
library(mediation)
report$log10GompRFlex = log10(report$GompRFlex)

med.fit = lm(n ~ GompGFlex, data=report)  #mediator ~ treatment
summary(med.fit)

out.fit = lm(log10GompRFlex ~ n + GompGFlex, data=report)  #outcome ~ mediator + treatment
summary(out.fit)

med.out <- mediate(med.fit, out.fit, treat = "GompGFlex", mediator = "n", robustSE = TRUE, sims = 100) #mediator test
summary(med.out)
